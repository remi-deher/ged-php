...
let accountForm;
let currentTenantIdForAccount;

function openModal(tenantId, account = null) {
    ...
    accountForm.innerHTML = `
        ...
        <div class="form-group">
            ...
            <div class="input-group"><span>🏷️</span><input style="padding-left: 30px;" type="text" id="account_name" name="account_name" value="${account?.account_name || ''}" required></div>
        </div>
        <div class="form-group">
            <label>Adresse e-mail</label>
            <div class="input-group"><span>@</span><input style="padding-left: 30px;" type="email" id="user_email" name="user_email" value="${account?.user_email || ''}" required></div>
        </div>
        <div class="form-group">
            <label>Imprimante par défaut pour ce compte</label>
            <select name="default_printer_id">${printerOptions}</select>
        </div>
        <hr>
        <div id="folder-selection-area">
            ...
            <button type="button" class="button" id="btn-list-folders">
                ...
                <span class="btn-text">🔗 Lier les dossiers</span>
            </button>
            <div class="feedback-message" id="folder-message"></div>
            <div class="folder-list" id="folder-list"></div>
        </div>
         <hr>
        <div id="automation-rules-area">
            <h3>🤖 Règles d'impression automatique</h3>
            <div id="rules-list"></div>
            <button type="button" class="button button-secondary" id="btn-add-rule">➕ Ajouter une règle</button>
        </div>
        <div class="form-actions">
            ${account ? `<form action="/settings/account/delete" method="POST" onsubmit="return confirm('Supprimer ce compte ?')"><input type="hidden" name="tenant_id" value="${tenantId}"><input type="hidden" name="account_id" value="${account.id}"><button type="submit" class="button button-delete">Supprimer</button></form>` : '<span></span>'}
            <button type="submit" class="button" id="btn-save-account">💾 Enregistrer</button>
        </div>
    `;
    ...
    accountForm.addEventListener('submit', (e) => prepareFormForSubmit(e));
...
async function listFolders(selectedMappings = []) {
...
function renderFolderList(foldersFromApi, selectedMappings = []) {
    const container = document.getElementById('folder-list');
    if (foldersFromApi.length === 0) {
        container.innerHTML = '<p style="text-align:center; color:#6c757d;">Aucun dossier retourné par l\'API.</p>';
        return;
    }
...
async function handleFolderCreation(e) {
    ...
    try {
        ...
        fd.append('folder_name', newName.trim());
...
function addRule(rule = null) {
    ...
    ruleDiv.innerHTML = `
        <div class="rule-header">
            <input type="text" class="rule-name" value="${rule?.rule_name || ''}" placeholder="Nom de la règle">
            <button type="button" class="button-icon button-delete btn-remove-rule">🗑️</button>
        </div>
        <div class="rule-body">
            <div class="rule-conditions"></div>
            <div class="rule-actions">
                <span><strong>Alors :</strong> Imprimer</span>
                <select class="rule-action-print">
                    <option value="attachments" ${rule?.action_print === 'attachments' ? 'selected' : ''}>les pièces jointes</option>
                    <option value="body" ${rule?.action_print === 'body' ? 'selected' : ''}>le corps de l'e-mail</option>
                    <option value="all" ${rule?.action_print === 'all' ? 'selected' : ''}>les deux</option>
                </select>
            </div>
            <button type="button" class="btn-add-condition">➕ Ajouter une condition</button>
        </div>
    `;
    ...
    ruleDiv.querySelector('.btn-add-condition').addEventListener('click', () => addCondition(condContainer));
}

function addCondition(container, condition = null) {
    const condDiv = document.createElement('div');
    condDiv.className = 'rule-condition';
    condDiv.innerHTML = `
        <span><strong>Si :</strong></span>
        <select class="rule-cond-field">
            <option value="from" ${condition?.field === 'from' ? 'selected' : ''}>l'expéditeur</option>
            <option value="subject" ${condition?.field === 'subject' ? 'selected' : ''}>l'objet</option>
        </select>
        <select class="rule-cond-operator">
            <option value="contains" ${condition?.operator === 'contains' ? 'selected' : ''}>contient</option>
            <option value="equals" ${condition?.operator === 'equals' ? 'selected' : ''}>est égal à</option>
        </select>
        <input type="text" class="rule-cond-value" value="${condition?.value || ''}" placeholder="valeur...">
        <button type="button" class="button-icon button-delete btn-remove-condition">❌</button>
    `;
...
function prepareFormForSubmit() {
    ...
    document.querySelectorAll('.automation-rule').forEach(ruleDiv => {
        const rule = {
            rule_id: ruleDiv.dataset.ruleId,
            rule_name: ruleDiv.querySelector('.rule-name').value,
            action_print: ruleDiv.querySelector('.rule-action-print').value,
            conditions: []
        };
        ruleDiv.querySelectorAll('.rule-condition').forEach(condDiv => {
            rule.conditions.push({
                field: condDiv.querySelector('.rule-cond-field').value,
                operator: condDiv.querySelector('.rule-cond-operator').value,
                value: condDiv.querySelector('.rule-cond-value').value
            });
        });
        rules.push(rule);
    });
    ...
    rulesInput.name = 'automation_rules';
...
export function init() {
    ...
    document.querySelectorAll('.btn-add-account').forEach(btn => btn.addEventListener('click', (e) => {
        e.stopPropagation();
        openModal(btn.dataset.tenantId);
    }));
    document.querySelectorAll('.btn-edit-account').forEach(btn => btn.addEventListener('click', (e) => {
        e.stopPropagation();
        openModal(btn.dataset.tenantId, JSON.parse(btn.dataset.account));
    }));
}
