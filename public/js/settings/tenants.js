...
function openModal(form, tenant = null) {
...
export function init() {
    ...
    if (tenantModal) {
        ...
        document.getElementById('btn-show-tenant-form')?.addEventListener('click', () => openModal(tenantForm));
        document.querySelectorAll('.btn-edit-tenant').forEach(btn => btn.addEventListener('click', (e) => {
            e.stopPropagation();
            openModal(tenantForm, JSON.parse(btn.dataset.tenant));
        }));
    }
}
